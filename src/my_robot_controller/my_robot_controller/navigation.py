...
class TurtleNavigationNode(Node):
   ...
   def __init__(self):
      ...
      # Defining three different positions to reach on map 
       self.goal_poses.append({'x': -3.2, 'y': 0.2, 'yaw': 60})
       self.goal_poses.append({'x': -1.7, 'y': -2.8, 'yaw': 30})
       self.goal_poses.append({'x': -8.2, 'y': 0.3, 'yaw': 0})
